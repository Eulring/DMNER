...
def collate_to_max_length(batch: List[List[torch.Tensor]]) -> List[torch.Tensor]:
    ...
    pad_match_mask_ct = torch.ones([batch_size, max_length, max_length]).float()
    for sample_idx in range(batch_size):
        data = batch[sample_idx][7]
        pad_match_mask_ct[sample_idx, : data.shape[1], : data.shape[1]] = data
    output.append(pad_match_mask_ct)
