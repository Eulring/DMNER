...
class MRCNERDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.data_use == 'train':
            start_positions, end_positions, start_positions_ct, end_positions_ct = self.combine_labels(data, mode='1')
    ...
    def combine_labels(self, data, mode='6'):
        sp0 = data['span_position']
        sp1 = data['span_position_gpt']
        sp2 = data['span_position_ap']
        sp3 = data['span_position_umls']
        sp4 = data['span_position_ds']

        if mode == '0':
            # keep all sp, and no mask
            sp = sp0 + sp1 + sp2 + sp3 + sp4
            spos = [int(e_.split(';')[0]) for e_ in sp]
            epos = [int(e_.split(';')[1]) for e_ in sp]
            return spos, epos, [], []

        if mode == '1':
            # keep only gold, mask other
            sp_pos = sp0
            sp_mask_ = sp1 + sp2 + sp3 + sp4
            sp_mask = [ele for ele in sp_mask_ if ele not in sp_pos]
        ...
        if mode == '3':
            # keep only gold, mask ds
            sp_pos = sp0
            sp_mask_ = sp2 + sp3 + sp4
            sp_mask = [ele for ele in sp_mask_ if ele not in sp_pos]
            spos = [int(e_.split(';')[0]) for e_ in sp_pos]
            epos = [int(e_.split(';')[1]) for e_ in sp_pos]
            smask = [int(e_.split(';')[0]) for e_ in sp_mask]
        ...
        if mode == '6':
            # keep only gold, mask other
            sp_23 = [ele for ele in sp2 if ele in sp3]
            sp_24 = [ele for ele in sp2 if ele in sp4]
            sp_34 = [ele for ele in sp3 if ele in sp4]
            sp_pos = sp0 + sp_23 + sp_34 + sp_24
            sp_mask_ = sp1 + sp2 + sp3 + sp4
            sp_mask = [ele for ele in sp_mask_ if ele not in sp_pos]
            spos = [int(e_.split(';')[0]) for e_ in sp_pos]
            epos = [int(e_.split(';')[1]) for e_ in sp_pos]
