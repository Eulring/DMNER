...
class BertLabeling(pl.LightningModule):
    ...
    def compute_loss(self, start_logits, end_logits, span_logits,
                     start_labels, end_labels, match_labels, start_label_mask, end_label_mask, match_mask_ct):
        ...
        if self.span_loss_candidates == "all":
            ...
            match_label_mask = match_label_mask.float() * match_mask_ct
            float_match_label_mask = match_label_mask.view(batch_size, -1)


        start_loss = self.bce_loss(start_logits.view(-1), start_labels.view(-1).float())
        start_loss = (start_loss * start_float_label_mask).sum() / (start_float_label_mask.sum() + 1e-9)
        end_loss = self.bce_loss(end_logits.view(-1), end_labels.view(-1).float())
        end_loss = (end_loss * end_float_label_mask).sum() / (end_float_label_mask.sum() + 1e-9)
    ...
    def training_step(self, batch, batch_idx):
        ...
        tokens, token_type_ids, start_labels, end_labels, start_label_mask, end_label_mask, match_labels, match_mask_ct, sample_idx, label_idx = batch
    ...
    def validation_epoch_end(self, outputs):
        ...
        print(f"EVAL INFO -> valid_f1 is: {span_f1}; precision: {span_precision}, recall: {span_recall}.")
